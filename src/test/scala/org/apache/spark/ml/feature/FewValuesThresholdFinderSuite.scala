...
class FewValuesThresholdFinderSuite extends FunSuite with BeforeAndAfterAll {
  ...
  test("Run few values finder on nLabels = 2 feature len = 3") {

    val finder = new FewValuesThresholdFinder(nLabels = 2, stoppingCriterion = 0, maxBins = 100, minBinWeight = 1)

    val feature: Array[(Float, Array[Long])] = Array(
      (4.0f, Array(1L, 2L, 3L)),
      (5.0f, Array(5L, 4L, 2L)) ,
      (4.0f, Array(3L, 20L, 12L))
    )
    val result = finder.findThresholds(feature)


    assertResult("-Infinity, 4.0, 5.0, Infinity") {
      result.mkString(", ")
    }
  }
  ...
  test("Run few values finder on nLabels = 6 feature len = 35662") {
    ...
    val finder = new FewValuesThresholdFinder(nLabels = 6, stoppingCriterion = 0, maxBins = 100, minBinWeight = 1)

    val result = finder.findThresholds(feature)

    assertResult(35662) {feature.length}
